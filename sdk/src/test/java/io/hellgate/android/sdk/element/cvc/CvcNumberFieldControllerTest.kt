...
class CvcNumberFieldControllerTest {
    class FieldValue {
        ...
        @Test
        fun `Invalid onValueChanged input, returns the same digits without invalid chars`() =
            runTest {
    ...
    class FieldStateTest {
        ...
        @Test
        fun `Invalid onValueChanged input, returns incomplete state`() =
            runTest {
                val controller = CvcNumberFieldController(flowOf(3))
        ...
        @Test
        fun `Invalid onValueChanged input, returns invalid state`() =
            runTest {
                val controller = CvcNumberFieldController(flowOf(3))
    ...
    class HasFocus {
        @Test
        fun `Focus change, Returns correct state`() =
            runTest {
                val controller = CvcNumberFieldController(flowOf(3))
    ...
    class VisibleError {
        ...
        @Test
        fun `Input values partially into textField and lose focus , Returns true`() =
            runTest {
                val controller = CvcNumberFieldController(flowOf(3))
