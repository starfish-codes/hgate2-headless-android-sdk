...
class ExpiryDateFieldTest : ComposeRoboTest() {
    @Test
    fun `Field is filled with 4 digit date from the past, No further input is accepted and field shows error`() =
        runTest {
    ...
    @Test
    fun `Valid Date 1,3,5 is typed in, Results in field with no error and valid state`() =
        runTest {
            val expiryDateField = ExpiryDateField()
            var expiryDateFieldState: FieldState? = null
            var onFocusedCount = 0
